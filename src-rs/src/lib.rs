...
pub fn convert_to_typeorm(json_str: &str) -> String {
    let data = helpers::parse_json(json_str);
...
/**
Convert parsed typeorm code to db-code compatible nodes.
*/
#[wasm_bindgen]
pub fn convert_from_typeorm_v2(program: &str) -> String {
    ...
    if let Some(body_array) = data["body"].as_array() {
        if body_array.len() > 0 {
            for node in body_array {
                let mut node_type = node["type"].to_string();
                node_type = helpers::trim_quotes(&node_type).to_string();

                if node_type == "ClassDeclaration".to_string() {
                    // Initialize a default table.
                    let mut table_object = helpers::get_default_table();

                    let mut table_name = node["id"]["name"].to_string();
                    table_name = helpers::trim_quotes(&table_name).to_string();
                    table_object["data"]["name"] = json!(table_name);

                    if let Some(attributes) = node["body"].get("body").unwrap().as_array() {
                        for attribute in attributes {
                            let mut column_object = helpers::get_default_column();

                            if let Some(decorators) =
                                attribute.get("decorators").unwrap().as_array()
                            {
                                let mut is_primary_key = false;
                                if let Some(primary_key) =
                                    column_object.get("primaryKey").unwrap().as_bool()
                                {
                                ...
                                for decorator in decorators {
                                    let mut decorator_name =
                                        decorator["expression"]["callee"]["name"].to_string();
                                    decorator_name =
                                        helpers::trim_quotes(&decorator_name).to_string();
                                    ...
                                    if decorator_name == "PrimaryGeneratedColumn" && !is_foreign_key
                                    {
                                        ...
                                        if let Some(arguments) = decorator["expression"]
                                            .get("arguments")
                                            .unwrap_or(&json!({}))
                                            .as_array()
                                        {
                                            if arguments.len() > 0 {
                                                let mut argument_type =
                                                    arguments[0]["type"].to_string();
                                                argument_type =
                                                    helpers::trim_quotes(&argument_type)
                                                        .to_string();
                                                if argument_type == "Literal" {
                                                    column_object["dataType"] =
                                                        json!(helpers::trim_quotes(
                                                            &arguments[0]["value"].to_string(),
                                                        ));
                                                }
                                            }
                                        } else {
                            ...
                            if let Some(data_type) =
                                column_object.get("dataType").unwrap_or(&json!("")).as_str()
                            {
                                if data_type.len() < 1 {
                                    column_object["dataType"] =
                                        json!(helpers::ts_type_extractor(attribute));
                                }
                            } else {
                                column_object["dataType"] =
                                    json!(helpers::ts_type_extractor(attribute));
    ...
    for table in &tables_vec {
        if let Some(columns) = table["data"].get("columns").unwrap().as_array() {
            for column in columns {
                if column["foreignKey"] != json!(null) {
                    if let Some(fk) = column.get("foreignKey").unwrap().as_object() {
                        ...
                        let mut target_table_name = column["foreignKey"]["target"]
                            .get("table")
                            .unwrap()
                            .to_string();

                        let mut target_column_name = column["foreignKey"]["target"]
                            .get("column")
                            .unwrap()
                            .to_string();
                        ...
                        {
                            if let Some(target_table_columns) =
                                target_table["data"].get("columns").unwrap().as_array()
                            {
                                if let Some(target_column) = target_table_columns.iter().find(|x| {
                                    x["name"] == helpers::trim_quotes(&target_column_name)
                                }) {
                                    let mut target_table_id =
                                        target_table.get("id").unwrap().to_string();
                                    let mut target_column_id =
                                        target_column.get("id").unwrap().to_string();

                                    target_table_id =
                                        helpers::trim_quotes(&target_table_id).to_string();
                                    target_column_id =
                                        helpers::trim_quotes(&target_column_id).to_string();

                                    target_table_name =
                                        helpers::trim_quotes(&target_table_name).to_string();
                                    target_column_name =
                                        helpers::trim_quotes(&target_column_name).to_string();
                                    ...
                                    tables_iter[table_idx]["data"]["columns"][column_idx]
                    ...
                    // filter for none and map to table id.
                }
